...
- Rewriting the codebase and splitting everything into their own packages for better code structure and readability. after that is done, my main focus is going to be to refine this as much as possible and then move on to implementing the to do list features one by one. alot of work ahead of me but i am excited to tackle them all. 
 - I will split all the commands into separate files within a CMD package, it will make addition alot easier but a great structure is necessary for that to work well.
## Progress: 
...
- split up the CMD functions to separate files, now main.go just calls the functions from the CMD package, this makes it easier to add new commands later on. surprisingly there was no issue within my code in the way that i thought there would be, everything worked flawlessly on the first try. The structure and architecture is still garbage but it is a start.
- refined the add function alot, now it handles errors better and i fixed some edge cases. it should work without much flaws now.
## Problems faced:
1. Karplogic.go <- gonna be split up to multiple files later on but for now its fine.
 ...
 - this week was a bit of a struggle, i had to rewrite alot of code and structure, seemed to be so overwhelming at first but i just got to it, i wanted to do the json parsing but now that i think about it, with the structure before and everything being in main it would be a nightmare, yesterday i spent 20 minutes just staring at my screen trying to fix a specific edge case within go, dead tired 5 am but i couldn't give it up, finally figured it out and fixed it, felt so good. but another issue popped and i fixed it today.
    - overall i am happy with the progress, i feel like i am learning alot about golang and structuring code, next week is gonna be all about adding features and refining the code even more. 
    - got much better with reading documentation and understanding how to use packages and libraries
    - next week i will focus on adding persistence to the program, saving/loading tasks from a json file. after that i will move on to adding more features one by one.